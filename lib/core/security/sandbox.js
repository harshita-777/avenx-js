...
try {
  Object.defineProperty(Function.prototype, 'constructor', {
    get() {
      throw new Error('Access to Function constructor is blocked for security reasons.');
    },
    configurable: true,
  });
} catch {
...
function wrapValue(val) {
  ...
  const traps = {
    ...
    get(target, key, receiver) {
      ...
      if (key === '__proto__') {
        throw new Error('Access to property "__proto__" is blocked for security reasons.');
      }
      if (key === 'constructor' && typeof target === 'function') {
        throw new Error('Access to property "constructor" on functions is blocked for security reasons.');
      }
    ...
    set(target, key, value, receiver) {
      if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
        throw new Error(`Writing to property "${String(key)}" is blocked for security reasons.`);
      }
...
export class AvenxSandbox {
  ...
  static validateSource(source) {
    ...
    if (typeof source === 'string' && FORBIDDEN_WORDS.test(source)) {
      throw new Error('Access to "constructor", "__proto__", or "prototype" is blocked for security reasons.');
    }
  }
  /**
